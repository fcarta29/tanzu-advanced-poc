...
10. See that the TSM data plane versions installed are different in both clusters: the one with thecurrent K8s version, and the other with the old version of K8s.

11. Upgrade theK8s version in the cluster with the old versionof K8s and upgrade the TSM data plane on it.
